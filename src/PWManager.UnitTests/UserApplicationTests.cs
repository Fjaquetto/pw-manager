...
public class UserApplicationTests
{
    ...
    public async Task GetUserByIdAsync_WhenExists_ReturnsMatchingUser()
    {
        ...
        _repositoryMock.Setup(r => r.GetUserByIdAsync(UserMock.Id1)).ReturnsAsync(expected);
    ...
    public async Task GetUserByIdAsync_WhenNotFound_ReturnsNull()
    {
        ...
        _repositoryMock.Setup(r => r.GetUserByIdAsync(nonExistentId)).ReturnsAsync((User?)null);
    ...
    {
        var user = UserMock.GetSingleUser();
        _repositoryMock.Setup(r => r.DeleteAsync(user)).Returns(Task.CompletedTask);
